...
export async function POST(request: Request) {
  ...
  // Orders are always created against a published instant-order service.
  // The agency and price are derived exclusively from the service itself —
  // never trusted from the client — so a caller cannot mint a fundable order
  // against an arbitrary agency at an arbitrary price.
  if (!serviceId) return jsonError('A service is required to create an order', 400)
