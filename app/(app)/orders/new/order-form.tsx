...
export default function OrderForm({ serviceId, agencyId, serviceMeta }: { serviceId: string | null; agencyId: string | null; serviceMeta?: { title: string; basePrice: number; currency: string } | null }) {
  const router = useRouter()
  const [title, setTitle] = useState(serviceMeta?.title ?? '')
  const [amount, setAmount] = useState(serviceMeta?.basePrice ? String(serviceMeta.basePrice) : '')
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-5 rounded-2xl border border-border bg-card p-6 surface-soft sm:p-8">
      <div>
        ...
        <h1 className="font-display mt-1 text-3xl font-semibold leading-tight tracking-tight sm:text-4xl">Tell the agent what you need</h1>
