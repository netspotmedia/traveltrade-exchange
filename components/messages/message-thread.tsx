...
/** Realtime chat thread for a single order, shared by the order detail page
 *  and the 2-pane messages inbox. */
export function MessageThread({
  orderId,
  currentUserId,
  scrollClassName = 'h-[calc(100dvh-16rem)] min-h-[26rem]',
}: {
  orderId: string
  currentUserId: string
  /** Height classes for the scrollable message area (varies by container). */
  ...
  return (
    <div className={cn('flex flex-col', scrollClassName)}>
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={cn('flex', mine ? 'justify-end' : 'justify-start')}>
              <div
                className={cn(
                  'flex max-w-[78%] flex-col gap-1 rounded-2xl px-4 py-2 text-sm glass-card',
